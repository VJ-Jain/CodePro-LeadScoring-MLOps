...
def encode_features():
    ...
    df_encoded.to_sql(name='features_inference', con=conn, if_exists='replace', index=False)
...
def get_models_prediction():
    ...
    df_new_data.to_sql(name='predicted_values', con=conn, if_exists='replace', index=False)
    conn.close()
...
def prediction_ratio_check():
    ...
    # read the input data
    conn = sqlite3.connect(DB_PATH + DB_FILE_NAME)
    df = pd.read_sql('select * from predicted_values', conn)
...
def input_features_check():
    ...
    # read the input data
    conn = sqlite3.connect(DB_PATH+DB_FILE_NAME)
    df = pd.read_sql('select * from features_inference', conn)
